...
sinceVersion: "@parity/product-sdk@0.17.0"
---

# Migrating to `@parity/truapi`

> **Applies to `@parity/product-sdk` v0.17.0 and later.** The switch to
> `@parity/truapi` landed in v0.17.0 — this guide is only needed when upgrading
> from an earlier version (≤ v0.16.x).
...
| Resource allocation | `truApi.requestResourceAllocation({ tag: "v1", value: [{ tag: "SmartContractAllowance", value: i }] })` → `res.value[0].tag` | `truApi.resourceAllocation.request({ resources: [{ tag: "SmartContractAllowance", value: { tag: "Index", value: i } }] })` → `res.outcomes[0]` |
...
const result = await truApi.resourceAllocation.request({
  resources: [{ tag: "SmartContractAllowance", value: { tag: "Index", value: derivationIndex } }],
});
result.match(
...
`SmartContractAllowance` no longer takes a bare number. Its payload is a tagged
`DerivationIndex`: `{ tag: "Index", value: n }` for a plain account index, or
`{ tag: "Raw", value: "0x…" }` for a raw 32-byte index as a hex string.
