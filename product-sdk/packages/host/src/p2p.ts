 ...
 *
 * Wraps the host's `p2pMedia` TrUAPI service (companion RFC
 * `TRUAPI_RFC_P2P_MEDIA.md`; Brevity plan `P2P_MEDIA_OVER_IROH_PLAN.md`
 * Phase 5). A p2p room is a session whose "relay" is a loopback endpoint the
 * host serves on `127.0.0.1` and whose remote party is a set of peers: the
 * dapp publishes/subscribes with plain `@moq/*` code pointed at the returned
 * loopback URL, while gossip discovery, hole-punching, and the media relay
 * live in the host.
 *
 * **Hand-vendored, pending upstream codegen.** The `p2pMedia` service is NOT
 * yet in the published `@parity/truapi` client (exactly as it is not yet in
 * the generated Dart host bindings — it is hand-vendored on both sides until
 * the upstream trait lands). So this module speaks the wire directly through
 * the shared {@link TrUApiTransport}: hand-authored SCALE codecs + the frame
 * ids from the host's `wire_table` (164–181 in the current vendored copy; the
 * RFC's upstream placeholders are 184+, assigned finally at merge). When
 * codegen lands, this module collapses to thin wrappers over
 * `truApi.p2pMedia.*`, identical in shape to {@link module:permissions}.
 *
 * @module
 */
...
// ---------------------------------------------------------------------------
// Wire ids — mirror the host `wire_table` p2pMedia block (Brevity plan §3).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SCALE codecs — byte-identical to the host's Dart codecs (types.dart).
// ---------------------------------------------------------------------------
...
/**
 * Map a transport `err` value — truapi ≥0.4's `CallError` envelope. `Domain`
 * carries the versioned `P2pError` and stays a typed {@link P2pError}; the
 * framework variants (`Denied` / `Unsupported` / `MalformedFrame` /
 * `HostFailure`) become a `HostCallFailedError`. Framework `Denied` is
 * deliberately NOT conflated with `P2pError.PermissionDenied` (nor framework
 * `Unsupported` with `P2pError.Unsupported`) — a domain error only ever
 * arrives inside `Domain`.
 */
function mapFailure(error: unknown, label: string): P2pFailure {
    if (error && typeof error === "object" && "tag" in error) {
        const e = error as { tag: unknown; value?: unknown };
        if (e.tag === "Domain") {
            // Domain wraps the versioned P2pError: { tag: "V1", value: <P2pError> }.
            let inner = e.value as { tag: string; value?: unknown } | undefined;
            if (
                inner &&
                typeof inner.tag === "string" &&
                /^V\d+$/.test(inner.tag) &&
                "value" in inner
            ) {
                inner = inner.value as { tag: string; value?: unknown };
            }
            if (inner && typeof inner.tag === "string") {
                return toP2pError(inner);
            }
        }
    }
...
async function call<Ok, Out>(
    ...
    // Response wire (truapi ≥0.4, matching the host's regenerated codecs):
    // [version][Result tag][CallError tag][inner version][P2pError…].
    const decodeResponse = (payload: Uint8Array) =>
        versioned(scale.Result(okCodec, scale.CallError(versioned(P2pErrorCodec)))).dec(payload)
            .value;
    return transport.request<Ok, unknown>({ ids, payload: reqPayload, decodeResponse }).match(
        (value) => ok(map(value)),
...
export async function roomEvents(
    ...
    const sub = transport.subscribeRaw({
        ...
        onInterrupt: (payload) => {
            // truapi ≥0.4 interrupt wire: an EMPTY payload is a graceful
            // server-side completion (no error); a non-empty payload is the
            // CallError envelope around the versioned P2pError.
            if (payload.length === 0) {
                interruptCallback?.();
                return;
            }
            const wire = versioned(scale.CallError(versioned(P2pErrorCodec))).dec(payload).value;
            interruptCallback?.(mapFailure(wire, "roomEvents interrupted"));
        },
...
if (import.meta.vitest) {
    ...
    // Response wire (truapi ≥0.4): versioned Result whose Err channel is the
    // CallError envelope around the versioned P2pError.
    const responseCodec = <T>(codec: Codec<T>) =>
        versioned(scale.Result(codec, scale.CallError(versioned(P2pErrorCodec))));
    const encOk = <T>(codec: Codec<T>, value: T) =>
        responseCodec(codec).enc({
            tag: "V1",
    ...
    const encErr = <T>(codec: Codec<T>, error: Parameters<typeof P2pErrorCodec.enc>[0]) =>
        responseCodec(codec).enc({
            ...
            value: {
                success: false,
                value: { tag: "Domain", value: { tag: "V1", value: error } },
            },
        });
    /** Encode a framework (non-Domain) CallError on the Err channel. */
    const encFrameworkErr = <T>(
        codec: Codec<T>,
        error:
    ...
    describe("public API over a fake transport", () => {
        ...
        test("a framework CallError (Denied) surfaces as HostCallFailedError, not a P2pError", async () => {
            const fake = fakeTransport({
                [IDS.roomCreate.request]: {
                    errBytes: encFrameworkErr(HostP2pRoomResponse, {
                        tag: "Denied",
                        value: undefined,
                    }),
                },
            ...
            await withTransport(fake, async (mod) => {
                ...
                expect(r.ok).toBe(false);
                if (!r.ok) {
                    expect((r.error as HostError).name).toBe("HostCallFailedError");
                    expect((r.error as HostError).message).toContain("Denied");
                }
            });
        });

        test("a framework CallError (HostFailure) carries the reason in the message", async () => {
            const fake = fakeTransport({
                [IDS.roomCreate.request]: {
                    errBytes: encFrameworkErr(HostP2pRoomResponse, {
                        tag: "HostFailure",
                        value: { reason: "node crashed" },
                    }),
                },
            });
            await withTransport(fake, async (mod) => {
                ...
                if (!r.ok) {
                    expect((r.error as HostError).name).toBe("HostCallFailedError");
                    expect((r.error as HostError).message).toMatch(/HostFailure: node crashed/);
                }
            });
        });
        ...
        test("roomEvents delivers mapped events then a typed interrupt", async () => {
            ...
            await withTransport(fake, async (mod) => {
                ...
                fake.interrupt(
                    versioned(scale.CallError(versioned(P2pErrorCodec))).enc({
                        tag: "V1",
                        value: {
                            tag: "Domain",
                            value: { tag: "V1", value: { tag: "RoomNotFound", value: undefined } },
                        },
                    }),
                );
        ...
        test("an empty interrupt payload completes the subscription without an error", async () => {
            const fake = fakeTransport({});
            await withTransport(fake, async (mod) => {
                const sub = await mod.roomEvents(7n, () => {});
                expect(sub).not.toBeNull();
                let interrupted = false;
                let interruptReason: unknown = "sentinel";
                sub?.onInterrupt((reason) => {
                    interrupted = true;
                    interruptReason = reason;
                });

                fake.interrupt(new Uint8Array());
                expect(interrupted).toBe(true);
                expect(interruptReason).toBeUndefined();
            });
        });

        test("a framework CallError interrupt surfaces as HostCallFailedError", async () => {
            const fake = fakeTransport({});
            await withTransport(fake, async (mod) => {
                const sub = await mod.roomEvents(7n, () => {});
                let interruptReason: unknown;
                ...
                fake.interrupt(
                    versioned(scale.CallError(versioned(P2pErrorCodec))).enc({
                        tag: "V1",
                        value: { tag: "HostFailure", value: { reason: "relay died" } },
                    }),
                );
                expect((interruptReason as HostError).name).toBe("HostCallFailedError");
                expect((interruptReason as HostError).message).toMatch(/HostFailure: relay died/);
            });
        });
    });
}
