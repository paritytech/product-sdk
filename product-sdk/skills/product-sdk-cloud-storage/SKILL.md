...
- **Environments**: `create({ environment })` accepts `"paseo"` (Paseo Next v2), `"summit"`, and `"devnet"` (public Paseo testnet) -- the `CloudStorageEnvironment` presets, keys of `CloudStorageNetworks`; all three currently available. `"polkadot"`/`"kusama"` are not yet available (compile error, and `getChainAPI` throws).
