...
```ts
type CloudStorageEnvironment = "paseo" | "devnet"   // keyof typeof CloudStorageNetworks
```

The `environment` shorthand accepted by `create(...)`. `"paseo"` (Paseo Next v2) and `"devnet"` (public Paseo testnet) are both currently available.
