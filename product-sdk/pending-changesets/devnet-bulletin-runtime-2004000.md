...
- `CloudStorageClient.renew()` builds the old `TransactionStorage` call via `@parity/bulletin-sdk`, so it will throw on devnet until that package is repointed at `DataRenewal.renew` (same caveat as #280).
